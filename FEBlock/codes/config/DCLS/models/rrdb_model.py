...
class B_Model(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        #
        p = self.model(lr)
        # p = F.interpolate(p, size=(target_height, target_width), mode='bilinear',
        #                   align_corners=False)
        print('p',p.shape)
        print('最爱邱宝宝了')
        # '''我加的'''
        # if p.size(2) != hr.size(2) or p.size(3) != hr.size(3):
        #     p = F.interpolate(p, size=(hr.size(2), hr.size(3)), mode='bilinear', align_corners=False)
        # '''上边'''
