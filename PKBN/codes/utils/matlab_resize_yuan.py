...
def deriveSizeFromScale(img_shape, scale):
    output_shape = []
    for k in range(2):
        output_shape.append(int(ceil(scale[k] * img_shape[k+2])))
    return output_shape
...
def imresizevec(inimg, weights, indices, dim):
    ...
    if dim == 0:
        weights = weights.reshape((wshape[0], wshape[2], 1, 1)) #xmz修改：末尾加了个1
        print(weights.shape)
        print(inimg[indices].shape)
        outimg =  np.sum(weights*((inimg[:,indices].squeeze(axis=1)).astype(np.float64)), axis=1)
        print(inimg[indices].shape)
    elif dim == 1:
        weights = weights.reshape((1, wshape[0], wshape[2], 1)) #修改同上
...
def imresize(I, scalar_scale=None, method='bicubic', output_shape=None, mode="vec"):
    ...
    indices = []
    for k in range(2):
        w, ind = contributions(I.shape[k+2], output_size[k], scale[k], kernel, kernel_width)
        print(I.shape[k+2])
        print(output_size[k])
