...
IMG_EXTENSIONS = ['.jpg', '.JPG', '.jpeg', '.JPEG', '.png', '.PNG', '.ppm', '.PPM', '.bmp', '.BMP'] # 图像文件的扩展名列表

def is_image_file(filename):
    return any(filename.endswith(extension) for extension in IMG_EXTENSIONS) # 判断文件名是否为图像文件

def _get_paths_from_images(path):
    '''get image path list from image folder''' #从图像文件夹获取图像路径列表
...
def _get_paths_from_lmdb(dataroot):
    ...
    return paths, sizes # 返回LMDB中的图像路径和大小信息
...
def _read_img_lmdb(env, key, size):
    '''read image from lmdb with key (w/ and w/o fixed size)  从lmdb的key读图像
    size: (C, H, W) tuple'''
...
def read_img(env, path, size=None):
    ...
    if env is None:  # img # 如果不是从LMDB读取
        img = cv2.imread(path, cv2.IMREAD_UNCHANGED) # 读取图像
    else:
        img = _read_img_lmdb(env, path, size) # 灰度图转为BGR
...
def augment(img, hflip=True, rot=True, mode=None): # 随机翻转和旋转逻辑
