'''定义了一个处理低质量和高质量图像对（LQ-GT pairs）的数据集类 LQGTDataset'''
import os
import random
import sys
...
class LQGTDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_GT = util.read_img(
            self.GT_env, GT_path, resolution # 读取图像
        ...
        if self.opt["phase"] == "train":
            ...
            img_LR = img_LR[rnd_h : rnd_h + GT_size, rnd_w : rnd_w + GT_size, :]
        ...
        # change color space if necessary  颜色空间转换
        if self.opt["color"]:
            H, W, C = img_LR.shape
