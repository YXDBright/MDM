...
import PKBN.codes.config.PKBN.options as option
from PKBN.codes.config.PKBN.models import create_model

'''model_ker'''

sys.path.insert(0, "../../")
import PKBN.codes.utils as util
from PKBN.codes.utils.file_utils import *
from PKBN.codes.data import create_dataloader, create_dataset
from PKBN.codes.data.data_sampler import DistIterSampler

from PKBN.codes.data.util import bgr2ycbcr
from PKBN.codes.data.util import imresize
...
def main():
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            model_ker.feed_data(
                LR_img, GT_img=train_data["GT"], lr=lr_t, lr_up=LR_up
            )
            ...
            if current_step % opt["logger"]["print_freq"] == 0:
                logs = model_ker.get_current_log()
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    # valid Predictor
                    # print("train", kernels.size())
                    model_ker.feed_data(LR_img, GT_img=val_data["GT"], lr_up=LR_up, ker_map=visuals_ker_val["ker"])
                ...
                # log
                logger.info(
                    "# Validation # PSNR: {:.6f}, Best PSNR: {:.6f},loss: {:.6f}| Iter: {}".format(avg_psnr, best_psnr,
                                                                                                   avg_loss, best_iter))
