...
def extract(a, t, x_shape):
    b, *_ = t.shape
    out = a.gather(-1, t)
    return out.reshape(b, *((1,) * (len(x_shape) - 1))) # 换了
...
class GaussianDiffusion(nn.Module):
    def __init__(self, denoise_fn, rrdb_net, timesteps=1000, loss_type='l1'):
        ...
        betas = cosine_beta_schedule(timesteps, s=0.008) #hparams[beta_s
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        # print(self.sqrt_recipm1_alphas_cumprod.shape)
        # print(noise.shape)
        return (
    ...
    def forward(self, ker_map, img_lr, img_lr_up, t=None, *args, **kwargs):
        x = ker_map
    ...
    def p_losses(self, x_start, t, cond, img_lr_up, noise=None):
        ...
        # print(img_lr_up.device)
        noise_pred = self.denoise_fn(x_tp1_gt, t, cond, img_lr_up).to(x_tp1_gt.device)
  ...
  #      duqv=1
    ...
    def res2img(self, img_, img_lr_up, clip_input=None):
        ...
        # img_lr_up = torch.nn.functional.interpolate(img_lr_up, size=(img_.size(2), img_.size(3)), mode='bilinear',
                                                    # align_corners=False) # 我加的
        #print('加之前3',img_.shape, img_lr_up.shape)
        # if img_.size(2) != img_lr_up.size(2) or img_.size(3) != img_lr_up.size(3):  # 加的
        #     img_lr_up = torch.nn.functional.interpolate(img_lr_up, size=(img_.size(2), img_.size(3)), mode='bilinear',
        #                                                 align_corners=False)
        #print('加之后4', img_.shape, img_lr_up.shape)
