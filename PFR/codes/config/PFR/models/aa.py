...
def b_Bicubic(variable, scale):  # 它的作用是将输入的variable进行双三次插值操作，并将插值后的结果返回
    B, C, H, W = variable.size()
    H_new = int(H / scale)
    W_new = int(W / scale)
    tensor_v = variable.view((B, C, H, W))
    re_tensor = imresize(tensor_v, 1 / scale)  # imresize函数的作用是进行图像的缩放操作。
    return re_tensor
...
class Gaussin_Kernel(object):  # 将Gaussin_Kernel类实例化，并将其赋值给self.gen_kernel属性。这个类用于生成模糊核。
    def __init__(self, kernel_size=24, blur_type='iso_gaussian',  #   kernel_size=24 iso_gaussian
                 sig=2.6, sig_min=0.2, sig_max=4.0, # sig_max=4.0
                 lambda_1=0.2, lambda_2=4.0, theta=0, lambda_min=0.2, lambda_max=4.0):  # 0.2   4.0
...
class BatchBlur(nn.Module): # 用于进行批量模糊操作
...
class SRMDPreprocessing(object):  # 进行超分辨率图像预处理
    def __init__(
        self,
        scale,
        kernel_size=24,  # 卷积核的大小，默认为24
        blur_type='iso_gaussian', # 模糊类型     aniso_gaussian
        theta=0,  # 旋转角度
        lambda_min=0.2,  # 控制生成模糊核时Lambda值的范围
        lambda_max=1.0,  # 4.0
        rate_iso=1.0, rate_cln=0.2, noise_high=0.01,random=True,lambda_1=0.1,lambda_2=0.2    # 0.08 1.2   2.4
    ):
        '''在`self.blur`的`BatchBlur`类中，`kernel_size`参数是用于指定模糊核的大小。模糊处理是通过将模糊核应用于图像的每个像素来实现的。模糊核的大小决定了模糊的程度和范围。

具体地说，`kernel_size`参数定义了一个正方形的模糊核的边长。较大的`kernel_size`会导致更强的模糊效果，因为更多的像素被考虑在内。反之，较小的`kernel_size`会产生较轻的模糊效果，因为只有少数像素被用来计算模糊值。

通过调整`kernel_size`的大小，你可以控制模糊处理的程度，以满足特定的需求。通常情况下，选择合适的`kernel_size`需要考虑图像的分辨率和期望的模糊效果。一般而言，较高分辨率的图像可能需要较大的`kernel_size`来保持适当的模糊效果，而较低分辨率的图像可能需要较小的`kernel_size`来避免过度模糊。

'''
        '''Lambda值表示模糊核的缩放因子或尺度参数，它决定了模糊核的大小。Lambda值越小，生成的模糊核越小；Lambda值越大，生成的模糊核越大。

具体来说，lambda_min和lambda_max参数定义了Lambda值的最小值和最大值的范围。生成模糊核时，会在这个范围内随机选择Lambda值，以产生不同尺度的模糊核。

当lambda_min和lambda_max的取值范围较小时，生成的模糊核尺度会比较接近；当取值范围较大时，生成的模糊核尺度会有较大的差异。这样可以通过调整Lambda值的范围来控制生成的模糊核的多样性和模糊程度。

总之，lambda_min和lambda_max参数影响生成的模糊核的尺度范围，从而影响最终的模糊效果。不同的Lambda值会导致生成的模糊核具有不同的大小和模糊程度。'''

        self.gen_kernel = Gaussin_Kernel( # 将Gaussin_Kernel类实例化，并将其赋值给self.gen_kernel属性。这个类用于生成模糊核。  模糊处理是通过将模糊核应用于图像的每个像素来实现的
        ...
        self.blur = BatchBlur(kernel_size=kernel_size)  # 将BatchBlur类实例化，并将其赋值给self.blur属性。这个类用于对图像进行批量模糊
    ...
    def __call__(self, hr_tensor, kernel=False):
        ...
        # blur
        hr_blured_var = self.blur(hr_var, b_kernels)  # 使用self.blur对hr_var进行模糊处理，得到模糊后的图像hr_blured_var


        # Down sample  # 如果scale不等于1，则使用b_Bicubic函数将模糊后的图像hr_blured_var和原始图像hr_var进行双三次插值，
        # 得到下采样后的图像lr_blured_t和lr_t。如果scale等于1，则将lr_blured_t赋值为hr_blured_var。
        if self.scale != 1:
            lr_blured_t = b_Bicubic(hr_blured_var, self.scale)
            lr_t = b_Bicubic(hr_var, self.scale)  # 似乎就是普通的双三次插值操作
        else:
...
def cal_sigma(sig_x, sig_y, radians):  # 用于计算协方差矩阵
...
def anisotropic_gaussian_kernel(batch, kernel_size, covar):  # 生成各向异性的高斯核
...
def isotropic_gaussian_kernel(batch, kernel_size, sigma):  # sigma高斯核的标准差
    ax = torch.arange(kernel_size).float().cuda() - kernel_size//2
    xx = ax.repeat(kernel_size).view(1, kernel_size, kernel_size).expand(batch, -1, -1)
    yy = ax.repeat_interleave(kernel_size).view(1, kernel_size, kernel_size).expand(batch, -1, -1)
    kernel = torch.exp(-(xx ** 2 + yy ** 2) / (2. * sigma.view(-1, 1, 1) ** 2))  # 高斯函数的定义
...
def stable_isotropic_gaussian_kernel(kernel_size=24, sig=4.0):
    ...
    k = isotropic_gaussian_kernel(1, kernel_size, x)  # 生成各向同性高斯核的函数。
    return k
...
def random_gaussian_kernel(batch, kernel_size=24, blur_type='iso_gaussian', sig_min=0.2, sig_max=4.0, lambda_min=0.2, lambda_max=4.0):
    if blur_type == 'iso_gaussian':  # 似乎这里可以做对比试验
