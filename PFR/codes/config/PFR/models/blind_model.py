...
from models.util_SRdiff import load_ckpt

logger = logging.getLogger("base") # 初始化日志对象



class B_Model(BaseModel):

    def build_model(self): # 定义B_Model类，继承自BaseModel

        hidden_size = 64
        dim_mults = '1|2|2|4'
        dim_mults = [int(x) for x in dim_mults.split('|')]  # 解析维度增幅配置
        rrdb_num_feat = 32

        denoise_fn = Unet(
            hidden_size, out_dim=3, cond_dim=rrdb_num_feat, dim_mults=dim_mults, flag = "ker")

        # RRDB网络配置
        rrdb_num_block=8
        rrdb = RRDBNet(3, 3, rrdb_num_feat, rrdb_num_block,
                           rrdb_num_feat // 2)

        load_ckpt(rrdb, self.rrdb_ckpt)
    ...
    def __init__(self, opt):
        ...
        self.rrdb_ckpt = opt['rrdb_ckpt']
        self.model = self.build_model()
        self.model = self.model.to(self.device)
        # self.model.GPU()
        if opt["dist"]: # 如果使用分布式训练
            self.rank = torch.distributed.get_rank()
        else:
            self.rank = -1  # non dist training  # 非分布式训练
        ...
        if self.is_train:
            ...
            wd_G = train_opt["weight_decay_G"] if train_opt["weight_decay_G"] else 0  # 获取权重衰减参数，没有则默认为0
            ...
            self.optimizer_G = torch.optim.Adam(
                [
                    {"params": params, "lr": train_opt["lr"]},
                ],
                weight_decay=wd_G,
                betas=(train_opt["beta1"],train_opt["beta2"]),
            )       

            self.optimizers = self.optimizers.append(self.optimizer_G)  # 将优化器添加到优化器列表
    ...
    #  这个方法接收输入数据，并准备模型需要的变量。这包括低分辨率图像LR_img，高分辨率图像GT_img等。
    # # 数据预处理函数，将数据移动到指定的设备上
        self.var_L = LR_img.to(self.device) # 低分辨率图像
    ...
    def optimize_parameters(self, step):
        ...
        # print(lr_up.device)
        ker_map = self.real_ker_map
        losses, _, _ = self.model(hr, lr, lr_up, ker_map)
    ...
    def test(self):
        ...
        ker_map = self.real_ker_map
        with torch.no_grad():
            self.fake_SR, _ = self.model.sample(lr, lr_up, hr.shape, ker_map)
