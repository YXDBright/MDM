...
class ImgMerger:
    # ImgMerger类用于合并图像块成一个完整图像。它将图像块根据位置信息存储，并在所有图像块到齐后合并。
...
# SSIM
##########
def gaussian(window_size, sigma): # 生成高斯窗函数
    gauss = torch.Tensor([exp(-(x - window_size // 2) ** 2 / float(2 * sigma ** 2)) for x in range(window_size)])
    return gauss / gauss.sum()


def create_window(window_size, channel): # 创建用于SSIM计算的2D高斯窗口
...
def _ssim(img1, img2, window, window_size, channel, size_average=True): # 计算两幅图像之间的结构相似性指数（SSIM）
...
class VGGFeatureExtractor(nn.Module): # VGGFeatureExtractor 类用于提取VGG网络特定层的特征。这些特征通常用于感知损失计算。
...
class PerceptualLoss(nn.Module): # PerceptualLoss 类实现了使用VGG网络特征的感知损失，用于比较两幅图像的高级特征差异。
    def __init__(self):
