...
from models.diffsr_modules_ker import Unet, RRDBNet
from models.diffusion_ker import GaussianDiffusion
...
class B_Model(BaseModel):
    def build_model(self):
        ...
        rrdb_num_feat = 32
        denoise_fn = Unet(
            hidden_size, out_dim=11, cond_dim=rrdb_num_feat, dim_mults=dim_mults, flag = "ker")
    ...
    def optimize_parameters(self, step):
        ...
        # print(lr_up.device)
        losses, _, _ = self.model(self.real_ker_map, lr, lr_up)
    ...
    def test(self):
        ...
        lr_up = self.lr_up 
        with torch.no_grad():
            self.fake_ker, _ = self.model.sample(lr, lr_up, self.real_ker_map.shape)
    ...
    def get_current_visuals(self):
        ...
        out_dict["ker"] = self.fake_ker.detach()
        out_dict["ker_real"] = self.real_ker_map.detach()
        # out_dict["Batch_SR"] = (
